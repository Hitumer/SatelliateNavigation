...
caCodesTable = zeros(32, settings.Ti*samplesPerCode);
 
%--- Find time constants --------------------------------------------------
ts = 1/settings.samplingFreq;   % Sampling period in sec
tc = 1/settings.chipFreq;  % C/A chip period in sec
...
for PRN = 1:32
    ...
    % millisecond).
    codeValueIndex = ceil((ts * (1:settings.Ti*samplesPerCode)) / tc);
...
%     codeValueIndex(end) = 1023;
    ...
    caCodesTable(PRN, :) = caCode( mod(codeValueIndex-1,settings.codeLength)+1 );

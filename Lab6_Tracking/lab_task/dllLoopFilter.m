% [y,h] = dllLoopFilter( x, h ) 
% The loop filter for the DLL with the current input x and the previously returned
% vector h
% 
% Inputs:
...
function [y,h] = dllLoopFilter( x, h )

zeta = 1/sqrt(2);
wn   = 8*2*zeta/(4*zeta^2+1);
Ti   = 1e-3;

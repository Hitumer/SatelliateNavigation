function hpol = GPSskyplot(varargin) 
%Function plots "sky view" from the receiver perspective.  
% 
%h = skyPlot(AZ, EL, PRN, stationName, line_style) 
% 
%   Inputs: 
...
elseif nargs == 3 
    ...
    line_style      = 'auto';     
elseif nargs == 4
    [az, el, prn, stationName] = deal(args{1:4}); 
    line_style      = 'auto';
else
    [az, el, prn, stationName, line_style] = deal(args{1:5});
end 
...
for elevation = 0 : 15 : 90 
    ...
    line(yunit * elevationSpherical, xunit * elevationSpherical, ... 
        'lineStyle', hstyle(elevation/15+1), 'color', tc, 'linewidth', 0.5, ... 
        'handlevisibility', 'off'); 
...
if ~isempty(stationName)
    title(sprintf('Skyplot of station %s',stationName));
end

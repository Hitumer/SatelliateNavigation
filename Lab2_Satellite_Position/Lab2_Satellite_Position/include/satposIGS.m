...
function x_s = satposIGS( time, year, dayNumber, PRN )
...
if ~exist(filename(1:end-2),'file')
    try
...
dt = zeros(1,length(trueVisibleSats));
...
t=0;
    for s=1:length(trueVisibleSats)
        ...
        dt(t+1,s) = neville(tx,measurem_points(:,4+4*(s-1)),time+t);
    end

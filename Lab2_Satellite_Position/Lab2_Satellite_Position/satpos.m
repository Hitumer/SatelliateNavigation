...
%SATPOS   Calculation of X,Y,Z coordinates at time t (seconds of GPS week)
%         for given ephemeris eph
% satp .. 3x1 vector containing the computed satellite position in ECEF (in meters)
...
% Extend the function to compute the satellite poairion at time t
% using the given eph
n= sqrt(GM/roota^6) + deltan;
...
i = i0 + idot * tk + cic*cos(2*theta) + cis* sin(2*theta);

RAAN_omega = Omega0 + (Omegadot-Omegae_dot)*tk - Omegae_dot*toe;
